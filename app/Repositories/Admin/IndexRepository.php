...
class IndexRepository {
    ...
    public function view_admin_index()
    {
        ...
        return view(env('TEMPLATE_ADMIN').'admin.index')
            ->with([
                'index_data'=>[],
                'consumption_data'=>[],
...
//            exit($e->getMessage());
    ...
    public function view_statistic_user($post_data)
    {
        ...
        $view_blade = env('TEMPLATE_ADMIN').'admin.entrance.statistic.statistic-user';
        return view($view_blade)->with($view_data);
    }
    // 【】流量统计
    public function view_statistic_item($post_data)
    {
        ...
        $view_blade = env('TEMPLATE_ADMIN').'admin.entrance.statistic.statistic-item';
        return view($view_blade)->with($view_data);
    }
...
//            exit($e->getMessage());
    ...
    public function view_user_client_list()
    {
        ...
        $agents = K_User::where(['userstatus'=>'正常','status'=>1])->whereIn('usergroup',['Agent','Agent2'])->orderby('id','desc')->get();

        $insufficient_clients = K_User::where(['userstatus'=>'正常','status'=>1,'usergroup'=>'Service'])->where('fund_expense_daily','>',0)
    ...
    public function get_user_client_list_datatable($post_data)
    {
        ...
        $query = K_User::select('*')
//        $query = K_User::select('id','pid','epid','username','usergroup','createtime')
//            ->whereHas('fund', function ($query1) { $query1->where('totalfunds', '>=', 1000); } )
            ->with('parent','ep','fund')
            ->withCount([
                'sites'=>function ($query) { $query->where('status',1)->whereIn('sitestatus',['优化中','待审核']); },
...
//        dd($list->toArray());
    ...
    public function operate_business_keyword_review($post_data)
    {
        ...
        if($keyword)
        {
            ...
            $keyword_owner = K_User::where("id",$keyword->createuserid)->lockForUpdate()->first();
            if($keyword_owner)
            {
