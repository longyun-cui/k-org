...
//        $this->model = new K_User;
    ...
    // 返回（后台）主页视图
    public function view_sponsor_index()
    {
        $me = Auth::guard("sponsor")->user();

        return view(env('TEMPLATE_ADMIN').'sponsor.index')
            ->with([
                'index_data'=>[],
                'consumption_data'=>[],
...
//            exit($e->getMessage());
    ...
    {
        $me = Auth::guard('sponsor')->user();
        return view(env('TEMPLATE_ADMIN').'sponsor.entrance.info.password-reset')->with(['data'=>$me]);
    }
    // 【基本信息】【密码】保存-数据
...
//            exit($e->getMessage());
    ...
    {
        return view(env('TEMPLATE_ADMIN').'sponsor.entrance.user.user-my-member-list')
...
//        dd($list->toArray());
    ...
    {
        return view(env('TEMPLATE_ADMIN').'sponsor.entrance.user.user-my-fans-list')
...
//        dd($list->toArray());
    ...
    {
        return view(env('TEMPLATE_ADMIN').'sponsor.entrance.user.relation-sponsor-list')
...
//            exit($e->getMessage());
    ...
    public function view_item_item_list($post_data)
    {
        ...
        $view_blade= env('TEMPLATE_ADMIN').'sponsor.entrance.item.item-list';
