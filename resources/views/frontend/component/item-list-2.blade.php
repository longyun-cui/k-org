...
>
    <div class="item-container bg-white">
        ...
        <figure class="image-container padding-top-2-3">
            <div class="image-box">
                <a class="clearfix zoom-" target="_blank"  href="{{ url('/item/'.$v->id) }}">
                    {{--<img class="grow" data-action="zoom-" src="{{ env('DOMAIN_CDN').'/'.$v->cover_pic }}" alt="Property Image">--}}
        ...
        <figure class="text-container clearfix">
            <div class="text-box">
                ...
                <div class="text-title-row multi-ellipsis-1 with-border-top">
                    <a href="{{ url('/org/'.$v->org->id) }}" style="color:#ff7676;font-size:13px;">
                        <img src="{{ url(env('DOMAIN_CDN').'/'.$v->org->logo) }}" class="title-portrait" alt="">
                        <c>{{ $v->org->name or '' }}</c>
                    </a>
                </div>
            </div>
        ...
        <span class="item-plus-box in-item-list-2 _none" role="button">
